	...
	static JFrame logged_in(String Card)
	{
		...
		Deposit.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				int value = JOptionPane.showConfirmDialog(null, message, "Enter in the correct information please", JOptionPane.OK_CANCEL_OPTION);

				if (value == JOptionPane.OK_OPTION)
				{
					String value1  = CardNo.getText();
					...
					try {
						String query = "Select Balance from Customer WHERE Card_No = ?";
					...
					try
					{
						String query = "UPDATE customer"+"SET balance = ?"+value2 + bal
								+"WHERE Card_No = ?";
						PreparedStatement statement = connection.prepareStatement(query);
